...
- これにより、備蓄登録時の入力負荷を下げつつ、通知判定や期限管理に必要な購入日情報を保持する
...
年間維持コストは、備えプランに含まれる各商品の単価、数量、賞味期限（月）をもとに、1年間で必要になる買い替え想定回数を加味して算出する。
...
- 賞味期限は `products.shelf_life_months` を用いる
- 小数点を含む場合は、合計値を表示時に四捨五入する
- 年間維持コストは保存値ではなく、表示時に算出する派生値として扱う

この方針により、商品マスタの賞味期限情報を利用して、ダッシュボードおよび備えプラン詳細画面で年間維持コストを表示する。
...
- 単価は `stock_items.unit_price` を用いる
- 数量は `stock_items.quantity` を用いる
...
- `plans.total_estimated_cost`
  - 初期費用の保存値として保持する
- 年間維持コスト
  - `products.price`、`plan_items.quantity`、`products.shelf_life_months` をもとに表示時に算出する
- 備蓄コスト目安
  - `stock_items.unit_price`、`stock_items.quantity` をもとに表示時に算出する

過剰な派生値保存を避けるため、年間維持コストと備蓄コスト目安はDBには保持せず、アプリケーション側で計算する。
