...
今回の追加対応として、備蓄品一覧画面（UI-009）に登録済み備蓄商品の編集機能を追加する。
...
- 登録済み備蓄商品の各カードは、表示兼編集フォームとして扱う
- 編集可能項目は `数量 / 単価 / 賞味期限` の3項目に限定する
- 商品名は表示のみとし、編集対象外とする
- 保存ボタンをカード下部に配置する
- 削除ボタンは保存ボタンの右側に配置する
- 保存ボタン押下時にのみ更新を反映する
- 保存中はローディング状態を表示する
- 更新失敗時はカード単位でエラー表示を行う
- 保存と削除の導線が混乱しないよう、同一カード内で役割を明確に分ける
...
- 備えプラン編集機能は、備えプラン詳細画面（UI-007）に追加機能として実装する
- 編集可能項目は `プラン名 / 人数 / 日数` に限定し、人数・日数変更時の再計算対象は `商品の数量のみ` とする

- 備蓄商品編集は、今回の追加対応として `数量 / 単価 / 賞味期限` のみを対象にする
- 商品名変更など、備蓄商品の全面的な編集機能は対象外とする
